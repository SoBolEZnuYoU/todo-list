...
export const EditButton = ({text, id}) => {
	const { setFormIsOpenFlag, setInputValue, setRequestValue, setEditingId} = use(TodoAppContext)

	return (
		<button className={styles.btn} type="button" onClick={() => {
			setEditingId(id)
			setRequestValue('edit')
			setInputValue(text)
			setFormIsOpenFlag(true)}
			}>
